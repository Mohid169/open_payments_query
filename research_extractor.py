...
def get_research_payments_for_physician(
    ...
    try:
        ...
        if 'Physician_First_Name' in sample_df.columns and 'Physician_Last_Name' in sample_df.columns:
        ...
        if len(group_totals) > 0:
            ...
            for npi in group_totals['NPI']:
                ...
                if pd.notna(row.get(first_name_col)):
                    ...
                    if not specialties_found and specialty_prefix in row:
                        if pd.notna(row.get(specialty_prefix)) and row.get(specialty_prefix).strip() != '':
                    ...
                    for i in range(1, 6):
                        ...
                        if first_col in row and last_col in row and pd.notna(row.get(first_col)) and pd.notna(row.get(last_col)):
            ...
            group_totals.rename(columns={payment_col: 'Total_Payment_USD'}, inplace=True)
